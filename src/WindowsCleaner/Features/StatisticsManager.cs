    ...
    public static class StatisticsManager
    {
        ...
        public static void RecordCleaningSession(CleaningStatistics stats)
        {
            try
            {
                ...
                Logger.Log(LogLevel.Info, LanguageManager.Get("log_statistics_saved", stats.FilesDeleted, stats.FormattedSize));
            }
            catch (Exception ex)
            {
                Logger.Log(LogLevel.Error, LanguageManager.Get("error_cleaning", "enregistrement statistiques", ex.Message));
            }
        }
...
<html>
<head>
    <title>{LanguageManager.Get("report_title")}</title>
    <meta charset=""utf-8"">
    <style>
        body {{
...
<body>
    <div class=""container"">
        <h1>📊 {LanguageManager.Get("report_title")}</h1>
        <p>{LanguageManager.Get("report_generated")} {DateTime.Now:dd/MM/yyyy à HH:mm:ss}</p>
        
        <h2>{LanguageManager.Get("report_global_stats")}</h2>
        <div class=""stats-grid"">
            <div class=""stat-card"">
                <h3>{LanguageManager.Get("report_space_freed")}</h3>
                <div class=""value"">{FormatBytes(totalBytes)}</div>
            </div>
            <div class=""stat-card"">
                <h3>{LanguageManager.Get("report_files_deleted")}</h3>
                <div class=""value"">{totalFiles:N0}</div>
            </div>
            <div class=""stat-card"">
                <h3>{LanguageManager.Get("report_cleaning_sessions")}</h3>
                <div class=""value"">{totalSessions}</div>
            </div>
        </div>
        
        <h2>{LanguageManager.Get("report_last_30_days")}</h2>
        <div class=""stats-grid"">
            ...
            <div class=""stat-card"" style=""background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);"">
                <h3>{LanguageManager.Get("report_cleanings")}</h3>
                <div class=""value"">{last30Days.Count}</div>
            </div>
        </div>
        
        <h2>🗂️ {LanguageManager.Get("report_app_cache_cleaning")}</h2>
        <div class=""cache-section"">
            <h3 style=""margin-top: 0;"">{LanguageManager.Get("report_global_stats_cache")}</h3>
            <div class=""cache-grid"">
                <div class=""cache-item"">
                    <strong>{LanguageManager.Get("report_total_cache_files")}</strong> {totalAppCacheFiles:N0}
                </div>
                <div class=""cache-item"">
                    <strong>{LanguageManager.Get("report_space_freed")}:</strong> {FormatBytes(totalAppCacheBytes)}
                </div>
            </div>
            
            <h3 style=""margin-top: 20px;"">{LanguageManager.Get("report_details_by_source")}</h3>
            <div class=""cache-grid"">
                <div class=""cache-item"">
                    <strong>VS Code:</strong> {vsCodeTotal:N0} {LanguageManager.Get("report_files")}
                </div>
                <div class=""cache-item"">
                    <strong>NuGet:</strong> {nugetTotal:N0} {LanguageManager.Get("report_files")}
                </div>
                <div class=""cache-item"">
                    <strong>Maven:</strong> {mavenTotal:N0} {LanguageManager.Get("report_files")}
                </div>
                <div class=""cache-item"">
                    <strong>npm:</strong> {npmTotal:N0} {LanguageManager.Get("report_files")}
                </div>
                <div class=""cache-item"">
                    <strong>{LanguageManager.Get("report_games")}</strong> {gameTotal:N0} {LanguageManager.Get("report_files")}
                </div>
            </div>
        </div>
        
        <h2>💿 {LanguageManager.Get("report_ssd_optimization")}</h2>
        <div class=""ssd-section"">
            <div style=""display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;"">
                <div>
                    <strong>{LanguageManager.Get("report_trim_optimizations")}</strong> {ssdOptimizationCount} {LanguageManager.Get("report_sessions")}
                </div>
                <div>
                    <strong>{LanguageManager.Get("report_smart_checks")}</strong> {diskHealthCheckCount} {LanguageManager.Get("report_sessions")}
                </div>
            </div>";
            ...
            if (lastSmartReport != null && !string.IsNullOrEmpty(lastSmartReport.DiskHealthReport))
            {
                ...
                html += $@"
            <h3 style=""margin-top: 20px;"">{LanguageManager.Get("report_last_smart_report")}</h3>
...
{escapedReport}
        ...
        <h2>📋 {LanguageManager.Get("report_session_history")}</h2>
        <table>
            <thead>
                <tr>
                    <th>{LanguageManager.Get("report_date")}</th>
                    <th>{LanguageManager.Get("report_profile")}</th>
                    <th>{LanguageManager.Get("report_files_deleted")}</th>
                    <th>{LanguageManager.Get("report_space_freed")}</th>
                    <th>App Cache</th>
                    <th>SSD</th>
                    <th>{LanguageManager.Get("report_duration")}</th>
                </tr>
            </thead>
            <tbody>
";
